AStar = require("./lib/astar");
    ...
    module.exports = Pathfinder = Class.extend({
        ...
        findPath: function(grid, entity, x, y, findIncomplete) {
            var start = [entity.x, entity.y],
        		end = [x, y],
        		path;

            this.grid = grid;
            //this.applyIgnoreList_(true);
            //log.info("AStar(this.grid, start, end)="+JSON.stringify(start)+","+JSON.stringify(end)+".");
        ...
        applyIgnoreList_: function(ignored) {
            ...
            _.each(this.ignored, function(entity) {
                x = entity.isMoving() ? entity.nextGridX : entity.x;
                y = entity.isMoving() ? entity.nextGridY : entity.y;
