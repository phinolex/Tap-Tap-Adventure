...
module.exports = Player = Character.extend({
    init: function(connection, worldServer, databaseHandler) {
        ...
        this.connection.listen(function(message) {
            ...
            log.info("Server/Clinet handshake initiated.");
...
//NOTE: Maybe a switch here?????
    ...
    questAboutItem: function(npcKind, questNumber, itemKind, callback){
        if(this.achievement[questNumber].found === true && this.achievement[questNumber].progress !== 999) {
    ...
    canEquipArmor: function(itemKind){
        ...
        if(armorLevel * 2 > this.level){
            this.server.pushToPlayer(this, new Messages.Notify("You need to be level " + armorLevel * 2 + " to equip this Armor."));
            return false;
        }
        return true;
    ...
    canEquipWeapon: function(itemKind){
        ...
        if(weaponLevel * 2 > this.level){
            this.server.pushToPlayer(this, new Messages.Notify("You need to be level " + weaponLevel * 2 + " to equip this Weapon."));
            return false;
        }
        return true;
    },
    ...
    handleInventoryPendant: function(itemKind, inventoryNumber){
        ...
        if((pendantLevel * 10) > this.level) {
            this.server.pushToPlayer(this, new Messages.Notify("You need to be level " + (pendantLevel * 10) + " to equip this pendant."));
            return;
        }
        var enchantedPoint = this.inventory.rooms[inventoryNumber].itemNumber;
    ...
    handleInventoryRing: function(itemKind, inventoryNumber){
        ...
        if((ringLevel * 10) > this.level) {
            this.server.pushToPlayer(this, new Messages.Notify("You need to be level " + (ringLevel * 10) + " to equip this ring."));
            return;
        }
        var enchantedPoint = this.inventory.rooms[inventoryNumber].itemNumber;
    ...
    handleInventoryBoots: function(itemKind, inventoryNumber){
        ...
        if((bootsLevel * 10) > this.level) {
            this.server.pushToPlayer(this, new Messages.Notify("You need to be level " + (bootsLevel * 10) + " to equop these boots."));
            return;
        }
    ...
    handleHurt: function(message){ // 9
        ...
        var mob = this.server.getEntityById(message[1]);
        if(mob &&
            (mob.kind === Types.Entities.FORESTDRAGON || mob.kind == Types.Entities.SEADRAGON || mob.kind == Types.Entities.HELLSPIDER || mob.kind == Types.Entities.SKYDINOSAUR)){
    ...
    handleSkill: function(message){
        ...
        if(type === "heal"){
            ...
            if(target && stunLevel > 0 && (now - this.stunExecuted) > 30 * 1000 && this.mana >= 150) {
            ...
            var now = (new Date).getTime();
            if(target && provocationLevel > 0 && (now - this.provocationExecuted) > 15 * 1000 && this.mana >= 50) {
