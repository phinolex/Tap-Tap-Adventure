  ...
  var InventoryHandler = Class.extend({
    ...
    setInventory: function(itemKind, inventoryNumber, number, itemSkillKind, itemSkillLevel){
      ...
      if(itemKind){
        ...
        $('#inventory' + inventoryNumber).css('background-image', "url('../client/img/2/item-" + Types.getKindAsString(itemKind) + ".png')");
        $('#inventory' + inventoryNumber).attr('title', Item.getInfoMsgEx(itemKind, number, itemSkillKind, itemSkillLevel, this.game.language));
        $('#sellInventory' + inventoryNumber).css('background-image', "url('../client/img/2/item-" + Types.getKindAsString(itemKind) + ".png')");
      ...
      if(this.game.storeDialog.visible) {
        this.game.storeDialog.inventoryFrame.inventories[inventoryNumber].assign(this.inventories[inventoryNumber].kind, this.inventories[inventoryNumber].count, this.inventories[inventoryNumber].skillKind, this.inventories[inventoryNumber]);
      }
    },
