define(['dialog', 'tabbook', 'tabpage', 'item'], function(Dialog, TabBook, TabPage, Item) {
    ...
    assign: function(datas) {
      ...
      if(player.pendant) {
        ...
        $('#characterItemPendant').attr(
          'title',
          Item.getInfoMsgEx(player.pendant, player.pendantEnchantedPoint, player.pendantSkillKind, player.pendantSkillLevel, game.language)
        );
      } else {
      ...
      $('#characterItemWeapon').attr(
        'title',
        Item.getInfoMsgEx(player.weapon, player.weaponEnchantedPoint, player.weaponSkillKind, player.weaponSkillLevel, game.language)
      );

      $('#characterItemArmor').css('background-image', 'url("img/2/item-' + Types.getKindAsString(player.armor) + '.png")');
      $('#characterItemArmor').attr(
        'title',
        Item.getInfoMsgEx(player.armor, 0, 0, 0, game.language)
      );
      ...
      if(player.ring) {
        ...
        $('#characterItemRing').attr(
          'title',
          Item.getInfoMsgEx(player.ring, player.ringEnchantedPoint, player.ringSkillKind, player.ringSkillLevel, game.language)
        );
      } else {
      ...
      if(player.boots) {
        ...
        $('#characterItemBoots').attr(
          'title',
          Item.getInfoMsgEx(player.boots, player.bootsEnchantedPoint, player.bootsSkillKind, player.bootsSkillLevel, game.language)
        );
      } else {
  ...
  var Skill = Class.extend({
    init: function(id, name, position, language) {
      this.background = $(id);
      this.body = $(id + 'Body');
      this.levels = [];
      this.name = name;
    ...
    setLanguage: function(language){
      this.body.attr('title', Types.Player.Skills.getComment(this.name, language));
    },
  ...
  var SkillPage = TabPage.extend({
    ...
    setSkill: function(name, level, language) {
