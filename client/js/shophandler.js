  ...
  var ShopHandler = Class.extend({
    init: function(game){
      ...
      $('#sellButton').click(function(){
        ...
        if(count > 0 && self.sellInventoryNumber !== null){
          $('#sellItem').css('background-image', "url('../client/img/2/inventory.png')");
      ...
      for(i=0; i<maxInventoryNumber && i<5; i++){
        ...
        $('#sellInventory' + i).click(function(){
          ...
          if(Types.isArmor(itemKind)){
            $('#sellItem').css('background-image', "url('../client/img/2/item-" + Types.getKindAsString(itemKind) + ".png')");
          } else{
            self.sellInventoryNumber = null;
          }
        });
    ...
    handleShop: function(message){
      ...
      for(i=0; i<message.length; i += 3){
        ...
        $('#buyItem'+slotNumber).css('background-image', "url('../client/img/2/item-" + Types.getKindAsString(this.buySlotItem[slotNumber]) + ".png')");
      ...
      for(i=0; i<this.game.player.maxInventoryNumber; i++){
        if(this.game.inventoryHandler.inventory){
          $('#sellInventory'+i).css('background-image', "url('../client/img/2/item-" + Types.getKindAsString(this.game.inventoryHandler.inventory[i]) + ".png')");
        }
      }
    },
  });
