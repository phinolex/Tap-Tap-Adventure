define(['../../dialog', 'interface/tabbook', 'interface/tabpage', 'entity/item/item', 'data/craftdata'], function(Dialog, TabBook, TabPage, Item, CraftData) {
    ...
    var StoreRack = Class.extend({
        ...
        assign: function(item) {
            ...
            this.basket.css('background-image', 'url("img/'+this.parent.scale+'/item-' + item.spriteName + '.png")')
            var itemDesc = item.name;
            var extraText = " - cost:";
            for (var k in item.input)
            {
            	extraText += "  " + ItemTypes.ItemData[k].name + " x" + item.input[k];
            }
    ...
    var CraftPage = TabPage.extend({
        init: function(id, itemType, items, scale) {
            ...
            this.items = [];
	    var i = 0;
	    for(var k in CraftData.Properties) {
		var craft = CraftData.Properties[k];
		if (craft.type == this.itemType)
		{
			var item = ItemTypes.ItemData[craft.output];
			if (item) {
			    this.items.push({
				index: i++,
				key: k,
				name: item.name ? item.name : '',
				kind: item.kind ? item.kind : 0,
				itemCount: 1,
				itemSkillKind: 0,
				itemSkillLevel: 0,
				input: craft.input,
				spriteName: item.spriteName != "" ? item.spriteName : item.key,
			    });
		        }
		        else
		        {
